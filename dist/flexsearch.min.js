...
function qa(a,b){a=a.length-b.length;return 0>a?1:a?-1:0}function pa(a,b){a=a[L];b=b[L];return a<b?-1:a>b?1:0}function oa(a,b){const c=L.length;for(let d=0;d<c;d++)a=a[L[d]],b=b[L[d]];return a<b?-1:a>b?1:0}function T(a,b,c){return a?{page:a,next:b?""+b:null,result:c}:c}function ha(a,b,c,d,e,f,g){let h,l=[];if(!0===c){c="0";var m=""}else m=c&&c.split(":");const p=a.length;if(1<p){const y=B(),r=[];let w,x;var n=0,k;let G;var u=!0;let C,D=0,N,da,X,ea;m&&(2===m.length?(X=m,m=!1):m=ea=parseInt(m[0],10));
if(g){for(w=B();n<p;n++)if("not"===e[n])for(x=a[n],G=x.length,k=0;k<G;k++)w["@"+x[k]]=1;else da=n+1;if(H(da))return T(c,h,l);n=0}else N=M(e)&&e;let Y;for(;n<p;n++){const ra=n===(da||p)-1;if(!N||!n)if((k=N||e&&e[n])&&"and"!==k)if("or"===k)Y=!1;else continue;else Y=f=!0;x=a[n];if(G=x.length){if(u)if(C){var t=C.length;for(k=0;k<t;k++){u=C[k];var A="@"+u;g&&w[A]||(y[A]=1,f||(l[D++]=u))}C=null;u=!1}else{C=x;continue}A=!1;for(k=0;k<G;k++){t=x[k];var z="@"+t;const Z=f?y[z]||0:n;if(!(!Z&&!d||g&&w[z]||!f&&
y[z]))if(Z===n){if(ra){if(!ea||--ea<D)if(l[D++]=t,b&&D===b)return T(c,D+(m||0),l)}else y[z]=n+1;A=!0}else d&&(z=r[Z]||(r[Z]=[]),z[z.length]=t)}if(Y&&!A&&!d)break}else if(Y&&!d)return T(c,h,x)}if(C)if(n=C.length,g)for(k=m?parseInt(m,10):0;k<n;k++)a=C[k],w["@"+a]||(l[D++]=a);else l=C;if(d)for(D=l.length,X?(n=parseInt(X[0],10)+1,k=parseInt(X[1],10)+1):(n=r.length,k=0);n--;)if(t=r[n]){for(G=t.length;k<G;k++)if(d=t[k],!g||!w["@"+d])if(l[D++]=d,b&&D===b)return T(c,n+":"+k,l);k=0}}else!p||e&&"not"===e[0]||
...
b,null,d,!0);r=null;w()})});if(c)f.then(c);else return f;return this}if(c)return this.add(a,b,null,d,!0),c(),this}b=this.encode(b);if(!b.length)return this;c=this.f;e=O(c)?c(b):b.split(this.split);this.filter&&(e=W(e,this.filter));const n=B();n._ctx=B();const k=e.length,u=this.threshold,t=this.depth,A=this.b,z=this.j,y=this.D;for(let r=0;r<k;r++){var g=e[r];if(g){var h=g.length,l=(y?r+1:k-r)/k,m="";switch(c){case "reverse":case "both":for(var p=h;--p;)m=g[p]+m,V(z,n,m,a,y?1:(h-p)/h,l,u,A-1);m="";
case "forward":for(p=0;p<h;p++)m+=g[p],V(z,n,m,a,y?(p+1)/h:1,l,u,A-1);break;case "full":for(p=0;p<h;p++){const w=(y?p+1:h-p)/h;for(let x=h;x>p;x--)m=g.substring(p,x),V(z,n,m,a,w,l,u,A-1)}break;default:if(h=V(z,n,g,a,1,l,u,A-1),t&&1<k&&h>=u)for(h=n._ctx[g]||(n._ctx[g]=B()),g=this.i[g]||(this.i[g]=ia(A-(u||0))),l=r-t,m=r+t+1,0>l&&(l=0),m>k&&(m=k);l<m;l++)l!==r&&V(g,h,e[l],a,0,A-(l<r?r-l:l-r),u,A-1)}}}this.c[f]=1;this.B=!1}return this};v.prototype.A=function(a,b,c){if(E(b))for(let l=0,m=b.length;l<m;l++){if(l===
m-1)return this.A(a,b[l],c);this.A(a,b[l])}else{const l=this.a.index,m=this.a.keys;var d=this.a.tag,e=this.a.id;let p;let n;for(var f=0;f<e.length;f++)p=(p||b)[e[f]];if(d){for(e=0;e<d.length;e++){var g=d[e];var h=g.split(":");for(f=0;f<h.length;f++)n=(n||b)[h[f]];n="@"+n}h=this.G[g];h=h[n]||(h[n]=[])}if("remove"===a){delete this.h[p];for(let k=0,u=m.length;k<u;k++){if(k===u-1)return l[m[k]].remove(p,c),this;l[m[k]].remove(p)}}e=this.a.field;h&&(h[h.length]=b);this.h[p]=b;for(let k=0,u=e.length;k<
u;k++){d=e[k];let t;for(g=0;g<d.length;g++)t=(t||b)[d[g]];d=l[m[k]];g="add"===a?d.add:d.update;k===u-1?g.call(d,p,t,c):g.call(d,p,t)}}return this};v.prototype.update=function(a,b,c){if(this.a&&J(a))return this.A("update",a,b);this.c["@"+a]&&M(b)&&(this.remove(a),this.add(a,b,c,!0));return this};v.prototype.remove=function(a,b,c){if(this.a&&J(a))return this.A("remove",a,b);var d="@"+a;if(this.c[d]){if(this.g)return this.m[this.c[d]].postMessage({remove:!0,id:a}),delete this.c[d],b&&b(),this;if(!c){if(this.async&&
...
let f=[],g=a;let h,l,m;if(J(a)&&!E(a)){c||(c=a.callback)&&(g.callback=null);l=a.sort;h=a.page;b=a.limit;var p=a.threshold;m=a.suggest;a=a.query}if(this.a){p=this.a.index;const y=g.where;var n=g.bool||"or",k=g.field;let r=n;let w,x;if(k)E(k)||(k=[k]);else if(E(g)){var u=g;k=[];r=[];for(var t=0;t<g.length;t++)d=g[t],e=d.bool||n,k[t]=d.field,r[t]=e,"not"===e?w=!0:"and"===e&&(x=!0)}else k=this.a.keys;n=k.length;for(t=0;t<n;t++)u&&(g=u[t]),h&&!M(g)&&(g.page=null,g.limit=0),f[t]=p[k[t]].search(g,0);if(c)return c(P.call(this,
a,r,f,l,b,m,y,h,x,w));if(this.async){const G=this;return new Promise(function(C){Promise.all(f).then(function(D){C(P.call(G,a,r,D,l,b,m,y,h,x,w))})})}return P.call(this,a,r,f,l,b,m,y,h,x,w)}p||(p=this.threshold||0);if(this.g)for(this.F=c,this.u=0,this.o=[],k=0;k<this.g;k++)this.m[k].postMessage({search:!0,limit:b,cursor:h,threshold:p,content:a});else{if(!d){if(this.async&&"function"!==typeof importScripts){let y=this;p=new Promise(function(r){setTimeout(function(){r(y.search(g,b,null,!0));y=null})});
if(c)p.then(c);else return p;return this}if(c)return c(this.search(g,b,null,!0)),this}if(!a||!M(a))return f;g=a;if(this.cache)if(this.B){if(c=this.l.get(a))return c}else this.l.clear(),this.B=!0;g=this.encode(g);if(!g.length)return f;c=this.f;c=O(c)?c(g):g.split(this.split);this.filter&&(c=W(c,this.filter));u=c.length;d=!0;e=[];var A=B(),z=0;1<u&&(this.depth&&"strict"===this.f?n=!0:c.sort(qa));if(!n||(t=this.i)){const y=this.b;for(;z<u;z++){let r=c[z];if(r){if(n){if(!k)if(t[r])k=r,A[r]=1;else if(!m)return f;
if(m&&z===u-1&&!e.length)n=!1,r=k||r,A[r]=0;else if(!k)continue}if(!A[r]){const w=[];let x=!1,G=0;const C=n?t[k]:this.j;if(C){let D;for(let N=0;N<y-p;N++)if(D=C[N]&&C[N][r])w[G++]=D,x=!0}if(x)k=r,e[e.length]=1<G?w.concat.apply([],w):w[0];else if(!m){d=!1;break}A[r]=1}}}}else d=!1;d&&(f=ha(e,b,h,m));this.cache&&this.l.set(a,f);return f}};v.prototype.find=function(a,b){return this.where(a,b,1)[0]||null};v.prototype.where=function(a,b,c,d){const e=this.h,f=[];let g=0;let h;var l;let m;if(J(a)){c||(c=
b);var p=Object.keys(a);var n=p.length;h=!1;if(1===n&&"id"===p[0])return[e[a.id]];if((l=this.I)&&!d)for(var k=0;k<l.length;k++){var u=l[k],t=a[u];if(!H(t)){m=this.G[u]["@"+t];if(0===--n)return m;p.splice(p.indexOf(u),1);delete a[u];break}}l=Array(n);for(k=0;k<n;k++)l[k]=p[k].split(":")}else{if(O(a)){b=d||Object.keys(e);c=b.length;for(p=0;p<c;p++)n=e[b[p]],a(n)&&(f[g++]=n);return f}if(H(b))return[e[a]];if("id"===a)return[e[b]];p=[a];n=1;l=[a.split(":")];h=!0}d=m||d||Object.keys(e);k=d.length;for(u=
0;u<k;u++){t=m?d[u]:e[d[u]];let A=!0;for(let z=0;z<n;z++){h||(b=a[p[z]]);const y=l[z],r=y.length;let w=t;if(1<r)for(let x=0;x<r;x++)w=w[y[x]];else w=w[y[0]];if(w!==b){A=!1;break}}if(A&&(f[g++]=t,c&&g===c))break}return f};v.prototype.info=function(){if(this.g)for(let a=0;a<this.g;a++)this.m[a].postMessage({info:!0,id:this.id});else return{id:this.id,items:this.length,cache:this.cache&&this.cache.s?this.cache.s.length:!1,matcher:aa.length+(this.v?this.v.length:0),worker:this.g,threshold:this.threshold,
